...
const handler = async (m, { conn, args }) => {
  ...
  const apiUrl = `https://api.nekorinn.my.id/ai/chatbot?ai_name=𝐀𝐧𝐠𝐞𝐥&text=${encodeURIComponent(text)}`;
  ...
  try {
    ...
    if (json && json.data) {
      await conn.sendMessage(m.chat, { text: `🤖 *𝐀𝐧𝐠𝐞𝐥 dice:* ${json.data}` }, { quoted: m });
    } else {
